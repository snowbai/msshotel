...
/**
 * 普通活动订单列表类
 * Class PromOrdinaryOrderList
 * @package common\models\order\order_lists
 */
class DinnerOrderList extends OrderList
{
    /**
     * 获取类型
     * @return array
    ...
    {
        $query = DinnerOrderAR::find()->select('*');
        //是否携带产品类型
        if ($with_product_type) {
            $query->addSelect(["CONCAT(".OrderConst::ORDER_DINNER.") as product_type"])->addSelect('dinner_type as product_subtype');
        }
        ...
        if ($this->_subtype !== null) $query->andWhere(['dinner_type' => $this->_subtype]);
        ...
        //无索引搜索项
        if (!empty($valid_search_data['dinner_type'])) $query->andWhere(['dinner_type' => $valid_search_data['dinner_type']]);
