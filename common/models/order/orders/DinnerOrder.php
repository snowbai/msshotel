...
use common\models\ar\OrderDinner as DinnerOrderAR;
 ...
 */
class DinnerOrder extends Order
{
    /**
     * 具体订单对象
     * @var DinnerOrderAR
    ...
    public static function getInstance($order_no, $order_id = 0, $order_ar = null, $order_search_ar = null)
    {
        ...
        if(empty($order_ar)){
            $order_ar = DinnerOrderAR::findOne(['id'=>(int)$order_search_ar->order_id]);
        }
        if(empty($order_ar)) return null;

        return new DinnerOrder($order_ar, $order_search_ar);
    }
    ...
    {
        if(empty($order_search_ar)) return null;
        $order_search_ar->product_type = OrderConst::ORDER_DINNER;
        $order_search_ar->product_subtype = isset($order_data['dinner_type']) ? $order_data['dinner_type'] : 0;
        $order_search_ar->save();

        $order_ar = new DinnerOrderAR();
    ...
    {
        return ['type'=>OrderConst::ORDER_DINNER];
    }
    ...
    public function getProductInfo($product_id)
    {
        ...
        $product_info['product_type'] = OrderConst::ORDER_DINNER;
        $product_info['product_subtype'] = $product_info['dinner_type'];
        $product_info['product_id'] = $product_info['dinner_id'];
    ...
    {
        $product_name = isset($new_product_info['dinner_name']) ? $new_product_info['dinner_name'] : $old_product_info['dinner_name'];

        $diff = '';
        if( isset($new_product_info['dinner_type']) &&
            ($new_product_info['dinner_type'] != $old_product_info['dinner_type'] || $new_product_info['dinner_id'] != $old_product_info['dinner_id']) )
        {
            $diff .= '购买餐饮'.$product_name.'修改为'.$new_product_info['dinner_name'].'; ';
        }
