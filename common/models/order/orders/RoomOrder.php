...
class RoomOrder extends Order
{
    ...
    public static function getInstance($order_no, $order_id=0, $order_ar=null, $order_search_ar=null)
    {
        ...
        if(empty($order_ar)){
            $order_ar = RoomOrderAR::findOne(['id'=>(int)$order_search_ar->order_id]);
        }
        if(empty($order_ar)) return null;

        return new RoomOrder($order_ar, $order_search_ar);
    }
    ...
    {
        if(empty($order_search_ar)) return null;
        $order_search_ar->product_type = OrderConst::ORDER_ROOM;
        $order_search_ar->product_subtype = isset($order_data['room_type']) ? $order_data['room_type'] : 0;
        $order_search_ar->save();

        $order_ar = new RoomOrderAR();
        $order_ar->setAttributes(array_merge($order_data,$products_data));
    ...
    public function getProductInfo($product_id)
    {
        ...
        $product_info['product_type'] = OrderConst::ORDER_ROOM;
        $product_info['product_subtype'] = $product_info['room_type'];
        $product_info['product_id'] = $product_info['room_id'];
    ...
    public function addNote($note)
    {
        ...
        $success = $this->_order_ar->save();
        if(!$success){
            $this->_pushError(3, 3000, '添加备注失败['.$this->_order_ar->order_no.']', $this->_order_ar->getErrors());
        }
    ...
    protected function _updateOrder($data)
    {
        ...
        if($this->_order_ar->save()){
            ...
            $this->_pushError(5, 2000, '更新订单信息失败', $this->_order_ar->getErrors());
            return false;
        }
    }
    ...
    protected function _setStatus($new_status)
    {
        ...
        if($this->_order_ar->save()){
            ...
            $this->_pushError(5, 2000, '更新订单状态失败', $this->_order_ar->getErrors());
            return false;
        }
    }
    ...
    {
        $product_name = isset($new_product_info['room_name']) ? $new_product_info['room_name'] : $old_product_info['room_name'];

        $diff = '';
        if( isset($new_product_info['room_type']) &&
            ($new_product_info['room_type'] != $old_product_info['room_type'] || $new_product_info['room_id'] != $old_product_info['room_id']) )
        {
            $diff .= '购买产品'.$product_name.'修改为'.$new_product_info['room_name'].'; ';
        }
