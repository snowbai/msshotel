...
class WechatController extends ApiController
{
    ...
    public function actionIndex()
    {
        ...
        // TODO 智能设备接口
        // TODO 多客服转发处理
        $hash = Medeen::getGetValue('hash');
        $request = Medeen::getRequest();
        $wechat = $this->findWechatByHash($hash);
        if (!$wechat->getSdk()->checkSignature()) {
            return 'Sign check fail!';
        }
        switch ($request->getMethod()) {
            case 'GET':
                if ($wechat->is_used == Wechat::STATUS_INACTIVE) { // 激活公众号
                    $wechat->updateAttributes(['is_used' => Wechat::STATUS_ACTIVE]);
                }
                return Medeen::getGetValue('echostr');
            case 'POST':
     ...
     * @throws NotFoundHttpException
     */
    protected function findWechatByHash($hash)
    {
        if (($model = Wechat::findOne(['hash'=>$hash],null,null,['select'=>['id','app_id','apapp_secret']])) !== null) {
            return $model;
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
    ...
    public function parseRequest()
    {
        ...
        if (empty($xml)) {
            Medeen::getResponse()->content = 'Request data parse failed!';
            Medeen::getApp()->end();
        }
        $message = [];
    ...
    public function resolveProcess()
    {
        ...
        foreach ($this->match() as $model) {
            ...
            $parts = Medeen::getApp()->createController($route);
            if (is_array($parts)) {
                ...
                $oldController = Medeen::getApp()->controller;
                $result = $controller->runAction($actionID);
                Medeen::getApp()->controller = $oldController;
            }
    ...
    {
        file_put_contents('2.txt',$this->message['MsgType'].'124\n',FILE_APPEND);
        ...
        return array_merge([
            ['route' => '/fans/subscribe'] // 默认所有请求都做一次关注请求处理
        ], $matches);
    }
    ...
    {
        return ['route' => 'api/DefalutController'];
